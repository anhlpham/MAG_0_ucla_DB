...
% Substitute parameters from input:
 if ~isempty(A.ParNames)
    for indp=1:length(A.ParNames)
       mag.setup = mag0_change_input(mag.setup,A.ParNames{indp},A.ParVal(indp));
       mag.param = mag0_change_input(mag.param,A.ParNames{indp},A.ParVal(indp));
       mag.farm  = mag0_change_input(mag.farm,A.ParNames{indp},A.ParVal(indp));
    end
 end
%----------------------------------------------------------------------
...
% Generate farm grid properties
 mag.farm = mag0_setup_farm(mag.param,mag.farm);
